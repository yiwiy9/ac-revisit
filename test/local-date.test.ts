...
test("LocalDateMath returns whole-day deltas across month and leap-year boundaries", () => {
  ...
  expect(dateMath.elapsedDays("2026-02-23", "2026-03-02")).toBe(7);
  expect(dateMath.elapsedDays("2024-02-28", "2024-03-01")).toBe(2);
  expect(dateMath.elapsedDays("2025-12-31", "2026-01-01")).toBe(1);
});

test("LocalDateMath centralizes the fixed 7-day due rule", () => {
  const dateMath = createLocalDateMath();

  expect(dateMath.isDue("2026-02-23", "2026-03-02")).toBe(true);
  expect(dateMath.isDue("2026-02-24", "2026-03-02")).toBe(false);
});

test("The review interval stays fixed at 7 days for MVP", () => {
  expect(REVIEW_INTERVAL_DAYS).toBe(7);
});
