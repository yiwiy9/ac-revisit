...
- Postconditions: 返却候補は必ず `reviewIntervalDays` 日以上経過済み
...
- `CandidateSelectionService` が `reviewIntervalDays` 未満を除外し、`reviewIntervalDays` 以上のみ抽出する
...
- ポップアップ header には `ac-revisit` のタイトルと close 操作だけが表示され、本文側に固定見出し「今日の一問」、状態説明の補助文、問題タイトル、主要アクションが縦に並ぶ
- 問題タイトルは今日の提案問題が未完了のときだけ通常リンクとして有効化され、それ以外は非活性テキストとして表示される
